...
class TwoPaneApp(wx.App):
    def __init__(self):
        ...
        btn = wx.Button(win, wx.ID_ANY, 'red')
        sizer.Add(btn)
        btn.Bind(wx.EVT_BUTTON, self.on_button)
        btn2 = wx.Button(win, wx.ID_ANY, 'white')
        sizer.Add(btn2)
    ...
    def on_button(self,event):
        ...
        color = event.GetEventObject().GetLabel()
        wx_colour = wx.Colour(color)
        self.window.my_clear = (wx_colour.Red() / 255, wx_colour.Blue() / 255,
                                wx_colour.Green() / 255, wx_colour.Alpha() / 255)
        self.window.Show()
