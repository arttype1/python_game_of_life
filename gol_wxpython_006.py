...
version 0.0.04 added save and open (with pickle)
version 0.0.05 added new and random game states
version 0.0.06 added population graph
"""
import wx
...
class PLC(wx.Panel):
    def __init__(self, parent, pos=(0,0), size=(0,0)):
        ...
        self.canvas.SetBackgroundColour('light gray')
        main_sizer.Add(self.canvas, 1, wx.EXPAND, 0)
        self.SetSizer(main_sizer)
        global data
        lines = PolyLine(data, legend='Red Line', colour='red', width=3)
        self.canvas.Draw(PlotGraphics([lines], "Population Graph", "Generation", "Population"), xAxis=(0, 100), yAxis=(0, 100))
        self.Layout()

    def on_idle(self, event):
        global data, pop, game_mode
        if game_mode == 'PLAY':
            lines = PolyLine(data, colour= 'green', width=3)
            x_max = max(100, len(data))
            y_max = max(200, max(y for x,y in data)+20)
            self.canvas.Draw(PlotGraphics([lines], "Population Graph",  "Generation", "Population"),
                             xAxis=(0, x_max), yAxis=(0, y_max))
...
class TwoPaneApp(wx.App):
    def __init__(self):
        ...
        self.filename = ''
        self.dirname = ''
        frame = wx.Frame(None, -1, "Game of Life: ", pos=(0, 0), size = (1660, 890),
        ...
        win3 = PLC(frame, pos=(10, 415), size=(800, 395))
        self.window1 = win
        self.window2 = win2
    ...
    def init_gui(self):
        menu_bar = wx.MenuBar()
        file_menu = wx.Menu()
        file_item_open = file_menu.Append(wx.ID_OPEN, 'Open', 'Open a game state from file')
        file_item_new = file_menu.Append(wx.ID_NEW, 'New', 'Create a new blank game state')
        file_item_rand = file_menu.Append(wx.ID_ANY, 'Random', 'Create a new random game state')
        file_item_save = file_menu.Append(wx.ID_SAVE, 'Save', 'Save the current game state')
        ...
        self.Bind(wx.EVT_MENU, self.on_new, file_item_new)
        self.Bind(wx.EVT_MENU, self.on_rand, file_item_rand)
        self.Bind(wx.EVT_MENU, self.on_save, file_item_save)
    ...
    def on_rand(self, event):
        global cells, count, data
        count = 0
        data = [[0, 0]]
        cells = [[0 for x in range(200)] for y in range(200)]
        x = 100
